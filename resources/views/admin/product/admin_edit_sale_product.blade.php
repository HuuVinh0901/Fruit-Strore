                        ...
                        <div class="card-body">
                            <div class="basic-form">
                                @foreach ($sale_product as $key1 => $show_sale_product)
                                    <form action="{{URL::to('admin_edit_update_sale_product/'.$show_sale_product->product_id)}}" method="POST" enctype="multipart/form-data"> {{-- gửi đến hàm --}}
                                        {{ csrf_field() }}
                                        ...
                                        <div class="form-row">
                                            <div class="form-group col-md-12">
                                                ...
                                                <input type="text" name="sale_product" class="form-control format_money" value="{{ $show_sale_product->product_sale}}">
                                            </div>
                                        </div>

                                        <button type="submit" name="add_sale" class="btn btn-primary w-25">Chỉnh sửa giá khuyến mãi</button>
                                        <a onclick="return confirm('Bạn có chắc chắc giá khuyến mãi này không?')"
                                                href="{{ URL::to('admin_delete_sale_product/' . $show_sale_product->product_id) }}"
                        ...
                        <div class="card-body">
                            <div class="basic-form">
                                @foreach ($sale_product as $key1 => $show_sale_product)
                                    ...
                                    @if($key1==$key2)
                                    <form action="{{URL::to('admin_edit_update_sale_product/'.$show_sale_product->product_id)}}" method="POST" enctype="multipart/form-data">
                                        {{ csrf_field() }}
